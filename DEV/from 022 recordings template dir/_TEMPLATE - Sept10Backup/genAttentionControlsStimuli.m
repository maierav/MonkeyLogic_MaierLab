...
% Target
target_diameter     = 3;    % dva, size of target
target_eccentricity = 8.5;    % dva
target_azimuth      = 12;   % polar degrees, location of 1st target in cartesian quad 1
target_sf           = .6;    % cycles / degree
target_tilt         = 180;    % grating tilt, counterclockwise, 0/180 = vertical, 90/360 horzontal
target_number       = 4;
...
for i = 1:length(genimages)
    ...
    % crop image (helps with ML)
     ...
     [nI nJ ~] = size(IMAGE);
      Imod = (64-mod(nJ,64))/2; 
      Jmod = (64-mod(nI,64))/2;
      IMAGE = padarray(IMAGE,[floor(Imod) floor(Jmod) 0],NaN,'post');
      IMAGE = padarray(IMAGE,[ceil(Imod) ceil(Jmod) 0],NaN,'pre');
