function [pixidx,center_x,center_y,end_x,end_y,m,b,idxrow,idxcol] = genNewBarwMidpt(mid_x,mid_y,pixperdeg,x,y,bar_tilt,bar_length,bar_width)
...
end_x = floor( (bar_length*(1/sqrt(1 + m.^2)) ).* pixperdeg) + mid_x; 
end_y = floor(((bar_length .* (m./sqrt(1 + m.^2))) ).* pixperdeg) + abs(mid_y);

center_x = -(ceil( (bar_length*(1/sqrt(1 + m.^2)) ).* pixperdeg) - mid_x);
center_y = - (ceil(((bar_length .* (m./sqrt(1 + m.^2))) ).* pixperdeg) - mid_y);
